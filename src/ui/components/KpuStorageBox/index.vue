...
<template>
  <KpuPopover class="p-0">
    ...
    <KpuIcon v-else name="i-ep:message-box" :size="16" />
    ...
    <template #panel>
      <div class="w-80">
        <div class="flex items-center justify-between px-4 py-3">
          ...
          <KpuIcon name="i-ep:plus" class="cursor-pointer text-base" @click="modalApi.open()" />
        </div>
        <KpuScrollArea :scrollbar="false" mask class="max-h-200px">
  ...
  <KpuModal title="请输入一个方便记忆的名称" content-class="min-h-auto">
    <KpuInput v-model="dialog.name" class="w-full!" />
  </KpuModal>
</template>
