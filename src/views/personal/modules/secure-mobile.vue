...
import { createCrudOptions, frontRules, getVerificationCode } from './secureMobile.tsx'
      ...
      <FsForm ref="formRef" class="p-0" v-bind="crudBinding.addForm">
        <template #form_code="{ form }">
          ...
          <InputGroup compact>
            ...
            <Button type="primary" :disabled="isStart" :loading="loading" class="w-[40%]" @click="() => getCaptcha(form.mobile)">
              {{ label }}
            </Button>
          </InputGroup>
        </template>
