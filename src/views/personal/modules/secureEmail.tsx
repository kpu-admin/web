...
import { updateEmailRequest } from '@/api/modules/profile/userInfo.ts'
...
export function getVerificationCode(formRef: Ref<any>) {
  ...
  async function getCaptcha(email: string) {
    if (!loading.value) {
      ...
      await form?.getFormRef().validateFields('email')
      if (!email) {
      ...
      startLoading()
      try {
        await sendSmsCode(email, MsgTemplateCodeEnum.EMAIL_EDIT)
