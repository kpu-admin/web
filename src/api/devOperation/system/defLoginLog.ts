...
import type {
  DefLoginLogPageQuery,
  DefLoginLogResultVO,
} from './model/defLoginLog.ts'
...
export function page(params: PageParams<DefLoginLogPageQuery>) {
  return requestClient.post<PageResult<DefLoginLogResultVO>>(`${ServicePrefix}/${MODULAR}/page`, params)
}
...
export function query(params?: DefLoginLogPageQuery) {
  return requestClient.post<DefLoginLogResultVO[]>(`${ServicePrefix}/${MODULAR}/query`, params)
}

export async function pageRequest(pageQuery: PageParams<DefLoginLogPageQuery>): Promise<UserPageRes> {
